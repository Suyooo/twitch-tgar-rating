<script lang="ts">
	...
	const socket = $derived(browser ? createSocket(params.roomCode, false) : undefined);
	const { pollActive, pollTotalVotes, pollAverage, pollPercentages } = $derived(pollHandler(socket));
	...
	$effect(() => {
		socket?.on("overlay-moved", (num) => (showStatus = num));
